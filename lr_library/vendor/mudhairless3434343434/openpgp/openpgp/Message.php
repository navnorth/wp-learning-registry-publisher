...
class Message implements \IteratorAggregate, \ArrayAccess
{
   ...
   */
    ...
    static function parse_string($input)
    {
        ...
        while (($length = strlen($input)) > 0) {
            if (($packet = \LearningRegistry\OpenPGP\Packet::parse($input))) {
                $msg[] = $packet;
            }
   ...
   */
    ...
    {
        $msg = $this;
        while ($msg[0] instanceof \OpenPGP\Packets\CompressedDataPacket) {
            $msg = $msg[0]->data;
        }
        ...
        foreach ($msg as $idx => $p) {
            if ($p instanceof \OpenPGP\Packets\LiteralDataPacket) {
            ...
            } elseif ($p instanceof \OpenPGP\Packets\PublicSubkeyPacket || $p instanceof \OpenPGP\Packets\SecretSubkeyPacket) {
                if ($userid) {
                ...
                $sigs = array();
                $subkey = $p;
            } elseif ($p instanceof \OpenPGP\Packets\PublicKeyPacket) {
                if ($userid) {
                    array_push($final_sigs, array($key, $userid, $sigs));
                    $userid = null;
                } elseif ($subkey) {
                ...
                $sigs = array();
                $key = $p;
            } elseif ($p instanceof \OpenPGP\Packets\UserIDPacket) {
                if ($userid) {
                    array_push($final_sigs, array($key, $userid, $sigs));
                    $userid = null;
                } elseif ($key) {
                ...
                $sigs = array();
                $userid = $p;
            } elseif ($p instanceof \OpenPGP\Packets\SignaturePacket) {
                $sigs[] = $p;
            }
        }
   ...
   */
    ...
    function verify_one($verifier, $sign, $sig)
    {
        if ($sign[0] instanceof \OpenPGP\Packets\LiteralDataPacket) {
            $sign[0]->normalize();
            $raw = $sign[0]->data;
        } elseif (isset($sign[1]) && $sign[1] instanceof \OpenPGP\Packets\UserIDPacket) {
            $raw = implode('', array_merge($sign[0]->fingerprint_material(), array(chr(0xB4),
            pack('N', strlen($sign[1]->body())), $sign[1]->body())));
        } elseif (isset($sign[1]) && ($sign[1] instanceof \OpenPGP\Packets\PublicSubkeyPacket || $sign[1] instanceof \OpenPGP\Packets\SecretSubkeyPacket)) {
            $raw = implode('', array_merge($sign[0]->fingerprint_material(), $sign[1]->fingerprint_material()));
        } elseif ($sign[0] instanceof \OpenPGP\Packets\PublicKeyPacket) {
